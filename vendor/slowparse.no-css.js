// Taken from:
//
// https://raw.githubusercontent.com/mozilla/thimble.webmaker.org/master/public/friendlycode/vendor/slowparse/slowparse.js
//
// And slightly modified to not parse CSS (search for NOCSS).
      ...
      while (!this.stream.end()) {
        if (this.containsAttribute(this.stream)) {
          ...
          if (!this.stream.end() && tagName === "style") {
            ...
            //NOCSS: var cssBlock = this.cssParser.parse();
            this._parseCDATA("style");
            this.domBuilder.pushContext("html", this.stream.pos);
            //NOCSS: this.domBuilder.text(cssBlock.value, cssBlock.parseInfo);
          }
